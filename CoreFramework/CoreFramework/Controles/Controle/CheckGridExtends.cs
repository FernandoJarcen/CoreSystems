...
{
    public partial class CheckGridExtends : DesignControlBase
    {
        ...
        public event EventHandler AoClicarLink;
        public event EventHandler AoMenuContexto;
        ...
        [Category("Custom")]
        [Description("Linha do componente editavel")]
        public bool Editavel
        {
            ...
            {
                _editavel = value;
                dgv_consulta.ReadOnly = _editavel;
            }
        }
        public int LinhaAtiva { get; set; }
        ...
        private void inicializar()
        {
            ...
            dgv_consulta.ReadOnly = Editavel;
        ...
        {
            bool valor = true;
            int qtdColuna = dgv_consulta.Columns.Count - 1;

            DataTable dtSelecionado = new DataTable();

            for (int indice = 1; indice <= qtdColuna; indice++)
            {
                String nomeColuna = Convert.ToString(dgv_consulta.Columns[indice].Name);
                if (dgv_consulta.Columns[indice].ValueType == null)
                    dtSelecionado.Columns.Add(nomeColuna, typeof(System.String));
                else
                    dtSelecionado.Columns.Add(nomeColuna, dgv_consulta.Columns[indice].ValueType);
            ...
            for (int indice = 0; indice < dgv_consulta.Rows.Count; indice++)
            {
                ...
                if (valor)
                {
                    ...
                    for (int idx = 1; idx <= qtdColuna; idx++)
                    {
        ...
        public int ExisteColuna(string nomeColuna)
        {
            int retorno = -1;
            for (int indice = 0; indice < this.dgv_consulta.Columns.Count; indice++)
            {
                string busca = this.dgv_consulta.Columns[indice].HeaderText;
                if (busca == nomeColuna)
                {
                    retorno = indice;
                    break;
        ...
        {
            dgv_consulta.Columns.Add(nome, nome);
        }
        ...
        public void VisibilidadeColuna(int coluna, bool parametro)
        {
            if (dgv_consulta.Columns.Count >= coluna)
                dgv_consulta.Columns[coluna].Visible = parametro;
        }
        ...
        public void AdicionarLink(int posicao, string header, string texto)
        {
            DataGridViewLinkColumn lnk = new DataGridViewLinkColumn();
            this.dgv_consulta.Columns.Insert(posicao, lnk);
            lnk.HeaderText = header;
            lnk.Name = this.dgv_consulta.Name + header;
            lnk.Text = texto;
            lnk.UseColumnTextForLinkValue = true;
            lnk.ActiveLinkColor = Color.White;
            lnk.LinkBehavior = LinkBehavior.SystemDefault;
            lnk.LinkColor = Color.Red;
            lnk.TrackVisitedState = true;
            lnk.VisitedLinkColor = Color.Red;
            dgv_consulta.CellEnter += dgv_consulta_CellEnter;
            this.dgv_consulta.DefaultCellStyle.Font = new Font("Microsoft Sans Serif", 10);
        }
        ...
        private void CheckDataGridExtends_Load(object sender, EventArgs e)
        {
            if (validarDados())
            {
                carregarDados(query);
        ...
        {
            int tamanhoCombo = tcb_action.Size.Width;
            int tamanhoTextbox = ttb_action.Size.Width;
            if (tamanhoCombo > 250)
                TamanhoCombo = 250;
            if (tamanhoTextbox > 250)
                TamanhoTexto = 250;
        }
        ...
        private void OnCliqueLink(object sender, DataGridViewCellEventArgs e)
        {
            if (this.AoClicarLink != null)
            {
                this.AoClicarLink(this, e);
            }
        }
        ...
        private void dgv_consulta_CellEnter(object sender, DataGridViewCellEventArgs e)
        {
            string tipo = this.dgv_consulta.Columns[e.ColumnIndex].GetType().Name;

            if (tipo == "DataGridViewLinkColumn")
            {
                OnCliqueLink(sender, e);
            }
        ...
        private void dgv_consulta_MouseClick(object sender, MouseEventArgs e)
        {
            if (e.Button == MouseButtons.Right)
            {
                if (this.AoMenuContexto != null)
                {
                    this.AoMenuContexto(this, e);
                }
            }
        }
